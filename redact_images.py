...
if not os.path.exists("logs"):
    os.makedirs("logs")
log_filename = f"logs/redact_images_{datetime.now().strftime('%Y%m%d_%H%M%S')}.log"
logging.basicConfig(
...
def load_redaction_specs(file_path):
    try:
        ...
        logger.warning(f"Error: Specification file '{file_path}' not found.")
        raise
...
def analyze_images(images, redaction_specs):
    """
    ...
    try:
        ...
        for page_number, image, xref in tqdm(images, desc="Detecting Entities in Images"):
            ...
            detected_entities = json.loads(response.choices[0].message.content).get("redactions", [])
        ...
        logger.warning("Error: 'config.json' or prompt file not found.")
        raise
    except Exception as e:
        logger.warning(f"Error during text detection in images: {e}")
        raise
            ...
            try:
                for item in image_list:
                    if item[0] == xref:
                        ...
                        break
                if not image_bbox:
                    logger.info(f"Skipping xref {xref} on page {page_number}: No bounding box found.")
                    continue

                page.add_redact_annot(image_bbox, fill=(0, 0, 0))
                page.apply_redactions()
            except ValueError:
                logger.info(f"Skipping xref {xref} on page {page_number}: Not a valid inline image.")

        doc.save(output_pdf, deflate=True)
        doc.close()
    except Exception as e:
        logger.warning(f"Error redacting images in PDF '{input_pdf}': {e}")
        raise
    ...
    try:
        for pdf_file in tqdm(os.listdir(input_folder), desc="Processing PDFs"):
            if pdf_file.endswith(".pdf"):
                ...
                redacted_entities = analyze_images(images, redaction_specs)
